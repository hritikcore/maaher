...
// Consolidate window.onload to initialize only once
window.onload = function () {
  redirectToMobile();
  initializeCart();

  // Only clear the cart if redirected back after payment success
  if (localStorage.getItem("paymentSuccess") === "true") {
    clearCartOnPaymentSuccess();
    localStorage.removeItem("paymentSuccess"); // Clear flag after use
  } else {
    updateCartCounter(); // Ensure cart counter updates correctly
  }
};
...
function checkout() {
  ...
  if (cart.length === 0) {
    ...
    // Set payment success flag for clearing cart later
...
// Clear cart and reset cart counter on payment success
function clearCartOnPaymentSuccess() {
  // Clear cart and total price from localStorage
  localStorage.removeItem("cart");
  localStorage.removeItem("totalPrice");
...
window.onload = function () {
  ...
  // Check payment success flag to clear cart
  if (localStorage.getItem("paymentSuccess") === "true") {
    clearCartOnPaymentSuccess();
